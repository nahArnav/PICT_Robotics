  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <div className="mx-auto max-w-[900px] px-6 py-10">
        ...
        <div className="rounded-2xl border border-line bg-paper-2 p-7 md:p-9">
          ...
          <div className="mt-6">
            {step === 0 && <div className="grid gap-5 sm:grid-cols-2"><Field label="Full name" required><input required value={profile.full_name} onChange={(event) => updateProfile('full_name', event.target.value)} className={inputCls} /></Field><Field label="PICT email"><input value={formData?.profile.email ?? ''} className={`${inputCls} bg-paper-2`} disabled /></Field><Field label="Phone" required><input required value={profile.phone ?? ''} onChange={(event) => updateProfile('phone', event.target.value)} className={inputCls} placeholder="98220 11234" /></Field><Field label="Branch" required><select value={profile.branch ?? ''} onChange={(event) => updateProfile('branch', event.target.value)} className={inputCls}><option value="" disabled>Select branch</option><option>Computer</option><option>IT</option><option>E&TC</option><option>Mechanical</option></select></Field><Field label="Division" required><select value={profile.division ?? ''} onChange={(event) => updateProfile('division', event.target.value)} className={inputCls}><option value="" disabled>Select division</option><option>A</option><option>B</option><option>C</option></select></Field><Field label="Year" required><select value={profile.year ?? 'FY'} onChange={(event) => updateProfile('year', event.target.value as 'FY' | 'SY')} className={inputCls}><option value="FY">First Year (FY)</option></select></Field></div>}
